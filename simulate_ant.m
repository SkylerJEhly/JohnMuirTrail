...
%disp(['string_controller: ' string_controller]);
for i=1:10
    for j=1:3
        %disp(['string_controller((i-1)*3+j)): ' string_controller((i-1)*3+j)]);
        fsm_controller(i,j) = str2double(string_controller((i-1)*3+j));
        %disp(['fsm_controller: ' mat2str(fsm_controller)]);
        %disp(['fsm_controller(i,j): ' fsm_controller(i,j)]);
    end
end
%disp(['fsm_controller: ' mat2str(fsm_controller)]);
fitness = 0;
...
for k=1:200
    ...
    % change currstate
    %disp(['curr_state: ' num2str(curr_state)]);
    %disp(['exist_food: ' num2str(exist_food)]);
    %disp(['fsm_controller: ' mat2str(fsm_controller)]);
